...
Here we develop the spectral approach in full. Following Connes' 2026 survey \cite{Connes2026}, we study the Weil quadratic form $\mathrm{QW}_\lambda$ and its even-odd decomposition. Our three main contributions are:
\begin{enumerate}
...
A recent survey by Connes \cite{Connes2026} reduces the Riemann Hypothesis to a spectral condition on the Weil quadratic form $\mathrm{QW}_\lambda$. We summarize the connection and present numerical evidence addressing the remaining open step.
...
\begin{lemma}[Simplicity of $\lambda_1^+$]
\label{lem:simplicity}
For all $33$ certificate values $\lambda \in [100,\, 1{,}300{,}000]$, the even-sector ground state is simple:
\[
...
Our numerical results address Connes' open problem (Section~6.6 of \cite{Connes2026}) from two angles:
\begin{enumerate}
...
\begin{proof}
Each summand has $\lambda_1(D_N(m\log p/L)) < 0$ by \Cref{thm:shift-parity} (since $N \geq 3$ and $0 < m\log p/L < 2$). The coefficients $c_m = (\log p)\,p^{-m/2} > 0$ are positive. We use a Rayleigh quotient argument: let $v^*$ be the unit eigenvector achieving $\lambda_1(D_N(\log p/L))$ for the dominant $m = 1$ shift. Then
\[
  \lambda_1(D_p) \leq (v^*)^T D_p\, v^* = c_1 \lambda_1(D_N(\log p/L)) + \sum_{m \geq 2} c_m\, (v^*)^T D_N(m\log p/L)\, v^*.
\]
The second sum satisfies $|\sum_{m \geq 2} c_m\, (v^*)^T D_N\, v^*| \leq \sum_{m \geq 2} c_m \|D_N\|_{\mathrm{op}} \leq C\,(\log p)/p$, which is negligible compared to $c_1 |\lambda_1| \geq C'\,(\log p)/\sqrt{p}$ for $p \geq p_0$. For small primes ($p = 2, 3, 5, 7$) the claim is verified by direct computation.
\end{proof}
...
\subsubsection{Lemma L3: Low--high coupling (historical)}

\begin{lemma}[Off-diagonal control --- superseded]
\label{lem:L3}
For $B = \mathrm{QW}[{:}K,\,K{:}]$ and $\lambda \in [30, 500]$, numerically $\|B\|_{\mathrm{op}}/L \in [0.23, 1.64]$.
\end{lemma}

\noindent\textbf{Note.} The original statement claimed $\|B\|_{\mathrm{op}} \leq \gamma L$ for all $\lambda$. As shown in \Cref{sec:L3-proof} below, the rigorous Hilbert--Schmidt bound gives $\|B\|_{\mathrm{op}} = O(\sqrt{\lambda}) = O(e^{L/2})$, which exceeds $O(L)$ asymptotically. The $O(L)$ scaling holds only on the tested range $\lambda \in [30, 500]$. \textbf{This lemma is not used in the proof of \Cref{prop:A6}}, which instead relies on the resolvent-damped framework (\Cref{sec:resolvent-M1,sec:leading-mode}) that controls the coupling \emph{differential} between sectors without requiring entry-wise bounds on $\|B\|$.
...
provides a direct bridge between zeros and primes. The Li coefficients correspond to the test function $h_n(s) = 1-(1-1/s)^n$. Connes \cite{Connes2026} constructs from this formula a self-adjoint operator $A_\lambda$ and reduces RH to the simplicity of its ground state with even eigenfunction (Theorem~6.1; see \Cref{sec:connes} for our numerical investigation). The prolate spheroidal wave operator, which commutes with the compressed Fourier transform, provides an analogy: its eigenvalues in the even sector are provably simple (Fact~6.3 in \cite{Connes2026}). Extending this simplicity to $A_\lambda$ is the remaining open problem.

Prior to the present work, the even-dominance property was an \emph{unproven assumption} in all approaches: Connes \cite{Connes2026} assumes it, and Connes--van~Suijlekom \cite{ConnesvS2025} assume it in their Carath\'eodory--Fej\'er truncation argument. Classical approaches via Krein--Rutman (positive kernels) or Sturm--Liouville (nodal theory) are not applicable because $A_\lambda$ has an \emph{indefinite} kernel. The present paper resolves this assumption via \Cref{prop:A6}.
...
The central analytical insight is the Leading-Mode Cancellation Lemma: the coupling difference between even and odd sectors is bounded by $O(1)$ while the diagonal advantage $D(\lambda)$ grows as $\sqrt{\lambda}$, producing a ratio $\rho(\lambda) \to 0$. Combined with the Euler--Maclaurin Proposition ($\rho^{\mathrm{EM}} < 0$ for $L \geq 13$, certified by interval arithmetic) and the PNT Transfer Lemma, this yields M1$''$ (resolvent subdominance) for all $\lambda \geq \lambda_0$.

In v1.1, both Lemma~B and Lemma~C have been upgraded to fully analytical arguments: the parity-split gap (Lemma~B, Steps~3--4) derives $c_{\mathrm{gap}} \geq 4\pi^2/L$ from Laplace asymptotics, and the sector-difference control (Lemma~C) replaces the global Neumann condition $\|R_0 K\| < 1$ by the tail-symmetry bound $|E_{\sin}^{\mathrm{tail}} - E_{\cos}^{\mathrm{tail}}|/D \leq C/N_0^2$. No numerical constants remain in the proof of Regime~2.
...
\begin{remark}[Galerkin truncation error and safety margins]
\label{rem:galerkin-safety}
The even block uses $k = 4$ modes in interval arithmetic (mpmath.iv, 50-digit precision); the Frobenius perturbation radius is $\mathrm{rad}_{\mathrm{frob}} \leq 0.0025$ at all certificate values. The odd block uses $N_0 = 40$--$90$ modes in float64, with the Cauchy tail bound from modes $N_0+1$ to $N_{\mathrm{big}} = 60$--$90$ bounding the truncation error. At $\lambda = 100$ (the worst case), the total odd-sector tail is $0.059$ (\texttt{sin\_info.total\_tail} in the certificate data). The combined Galerkin error is thus $\leq 0.0025 + 0.059 = 0.061$, while $|\mathrm{cert\_gap}| = 2.11$---a safety factor of $\mathbf{34\times}$. For $\lambda \geq 200$, the safety factor exceeds $65\times$ and grows monotonically. The Galerkin truncation is therefore negligible relative to the certified gap at every certificate value.
\end{remark}
...
\begin{corollary}[Explicit gap bound]
...
\]
The earlier numerical estimate $c_{\mathrm{gap}} \geq 0.5$ is thus conservative by a factor $\geq 5$.
\end{corollary}

\emph{Step~4 (Cross-term control --- via orthogonality, not Laplace).} This step controls the \emph{off-diagonal} couplings $B_{1j}$ by a mechanism entirely different from Step~3. For odd $j \geq 3$, the cross-overlap $S^+(1,j;\delta,L)$ vanishes at $\delta = 0$ by orthogonality of the cosine basis on $[-L,L]$ (Step~1). At the Laplace endpoint $\delta = L$: $S^+(1,j;L,L) = (-1)^j\delta_{1j}/2 = 0$ for $j \neq 1$ (again by orthogonality, now on $[0,L]$). Since the cross-overlap vanishes at \emph{both} endpoints of the prime-sum concentration, the coupling $B_{1j}$ comes from the subleading Laplace term, giving $|B_{1j}| = O(j\sqrt{\lambda}/L)$ instead of $O(\sqrt{\lambda})$. The ratio:
\[
...
\emph{Step~5 (Summation and difference).} The energy contribution from modes $j \geq 2$ satisfies $\sum_j |B_{1j}|^2/g_j = O(\sqrt{\lambda})$ in each sector individually. The \emph{difference} between sectors is controlled by the Leading-Mode Cancellation (\Cref{lem:leading-cancel}): the coefficients $c_{\sin}(L) - c_{\cos}(L) = O(1/L^2)$. Therefore:
\[
...
For fixed truncation dimension $N_0$ (with $N_{\cos} = 4$, $N_{\sin} = 40$ in practice), the tail modes $j > N_0$ contribute nearly equally to both sectors. The tail correction to the eigenvalue \emph{difference} satisfies:
\begin{equation}
\label{eq:tail-diff}
...
Additionally, the tail-restricted resolvent satisfies $\|R_0^{\mathrm{tail}} K^{\mathrm{tail}}\|_{\mathrm{op}} \leq L/300 \ll 1$ for all $\lambda \geq 100$ (Schur test on $j, k > N_0$), so the Neumann series converges on the tail unconditionally.
\end{lemma}

\begin{proof}
\emph{Step~1 (Tail contribution per mode).} For $j > N_0$, each mode contributes $|B_j^{\pm}|^2/g_j^{\pm}$ to $E_{\pm}$. By the parity-split gap bound (\Cref{lem:higher-mode-decay}), $g_j \geq 4\pi^2(j^2-1)\sqrt{\lambda}/L^2$. The coupling satisfies $|B_j^{\pm}| \leq 8\pi\sqrt{\lambda}/L$ (from Step~2 of \Cref{lem:higher-mode-decay}). Therefore:
\[
  \frac{|B_j^{\pm}|^2}{g_j^{\pm}} \leq \frac{64\pi^2\lambda/L^2}{4\pi^2(j^2-1)\sqrt{\lambda}/L^2}
  = \frac{16\sqrt{\lambda}}{j^2-1}.
\]

\emph{Step~2 (Tail sum is parity-symmetric).} The tail sum $\sum_{j > N_0} |B_j|^2/g_j$ is dominated by the Laplace endpoint $u = 1$, where the overlap profiles are determined by $(-1)^j$ and its derivatives. Both the even and odd sectors have the same tail asymptotics up to $O(1/L)$ corrections, because the parity of the \emph{leading mode} ($n = 1$ for cos, $n = 0$ for sin) affects only the low modes, not the tail.

\emph{Step~3 (Sector difference).} The difference between tail contributions is:
\[
...
\emph{Step~4 (Tail Schur test).} For $j, k > N_0$, $j \neq k$:
\[
...
\]
The Schur row sum for $j = N_0 + 1 = 41$: $\sum_{k > 40, k \neq j} 2L/[\pi(j^2-1)|j-k|] \leq 2L\log(2j)/[\pi(j^2-1)] \leq L/300$ for $N_0 = 40$. For $L \leq 300$ (i.e., all practical~$\lambda$), $\|R_0^{\mathrm{tail}} K^{\mathrm{tail}}\| < 1$.
\end{proof}
...
\[
  ...
  = O(L^2/N_0^2).
\]
For $N_0 = 40$: the truncation error is $\leq L^2/1600 \leq 0.12$ at $L = 14$, which is negligible compared to $|\mathrm{cert\_gap}|$ at all certificate values.
\end{lemma}
\begin{proof}
The coupling $\|B_{\mathrm{tail}}\|_{\mathrm{op}} \leq 8\pi\sqrt{\lambda}/L$ (Step~2 of \Cref{lem:higher-mode-decay}) and the gap to the $(N_0+1)$-th mode satisfies $g_{N_0+1} \geq 4\pi^2 N_0^2\sqrt{\lambda}/L^2$ (\Cref{cor:explicit-gap}). By the Schur complement formula for the eigenvalue perturbation, the correction is bounded by $\|B_{\mathrm{tail}}\|^2/(g_{N_0+1} - \|B_{\mathrm{tail}}\|)$. At $N_0 = 40$, $L = 14$: the numerator is $\leq 16\sqrt{\lambda}/1599$ and the denominator $\geq 4\pi^2 \cdot 1600\sqrt{\lambda}/196 - O(L) \gg $ numerator.
\end{proof}
...
\]
Then $\rho^{\mathrm{EM}}(L)$ is monotonically decreasing for $L \geq 7$ and crosses zero at $L \approx 12.6$. For $L \geq 13$:
\[
...
\begin{proof}
The integrals defining $B_j^{\mathrm{EM}}$ and $g_j^{\mathrm{EM}}$ are evaluated in interval arithmetic (mpmath.iv, 60-digit precision, 48-point Gauss--Legendre quadrature) at a grid of $L$~values. The certified intervals have width $\sim 2 \times 10^{-14}$:
...
\medskip\noindent
The zero-crossing occurs between $L = 12.5$ and $L = 13$, at approximately $L \approx 12.57$. The monotonicity follows from the Laplace structure: as $L \to \infty$, all integrals are dominated by the endpoint $u = 1$, where the even and odd sectors become identical (both have $f(1) = \pm 1/2$ with the same pattern), so the energy difference vanishes relative to~$D$. The certified grid confirms strict monotone decrease for $L \geq 7$.
\end{proof}
...
\]
where $\theta(x) = x + E(x)$ with $|E(x)| < x/(20\log x)$ for $x \geq 32{,}299$ (Dusart~\cite{Dusart2010}, Theorem~6.4; the classical form $|E(x)| \leq C_0\,x\,e^{-c\sqrt{\log x}}$ from \cite{IwaniecKowalski2004}, Theorem~6.9, also suffices). Integration by parts on $\Delta$:
\[
...
\begin{corollary}[M1$''$ is proved --- explicit threshold]
\label{cor:M1-proved}
Assumption~\ref{M1pp} holds with the explicit threshold $\lambda_0 = 442{,}413$ (i.e., $L_0 = 13$): for all $\lambda \geq \lambda_0$ and any $\eta \in (0,1)$,
\[
  E_{\sin}(\lambda) - E_{\cos}(\lambda) \leq (1-\eta)\,D(\lambda).
\]
\end{corollary}
...
\begin{proof}
By \Cref{prop:euler-maclaurin}, $\rho^{\mathrm{EM}}(13) = -0.0109$ (certified by interval arithmetic with precision $\sim 2 \times 10^{-14}$).

It remains to verify that the PNT transfer error is smaller than this margin. Using the explicit Dusart bound \cite{Dusart2010} $|\theta(x) - x| < x/(20\log x)$ for $x \geq 32{,}299$, the error in \Cref{lem:pnt-transfer} at $L = 13$ evaluates to:
\[
  |\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| \leq \frac{C \cdot L}{20\,\log\lambda} = \frac{C}{20} < 0.005
\]
for $\lambda \geq e^{13} = 442{,}413$, where $C < 2$ accounts for the ratio structure of the resolvent energies (each $|B_j|^2/g_j$ error is bounded by $\Delta B \cdot (2|B| + \Delta B)/g$, with $\Delta B/B < 1/(20L)$ from the Dusart bound). Since $0.005 < 0.0109/2$, we obtain:
\[
  \rho(\lambda) < \rho^{\mathrm{EM}}(13) + 0.005 < -0.0109 + 0.005 = -0.006 < 0 \quad\text{for all } \lambda \geq 442{,}413.
\]
Since $\rho < 0$ means $E_{\sin} < E_{\cos}$, the claim follows with $\lambda_0 = 442{,}413$. This threshold lies well within the overlap zone with Regime~1 (certificates up to $\lambda = 1{,}300{,}000$), providing a safety margin of nearly one order of magnitude.
\end{proof}
...
Between adjacent certificate values, the certified gap cannot cross zero. The argument does \emph{not} rely on eigenvalue additivity (which fails in general); instead, it combines three rigorous ingredients that each address a different mechanism of change:

\begin{enumerate}[label=(\roman*)]
  \item \textbf{New primes (Rayleigh-quotient argument).}
    When $\lambda$ crosses a prime~$p$, the rank-one update $W_p$ is added to both sectors.
    By the Shift Parity Lemma (\Cref{thm:shift-parity}), $\lambda_1(W_p^+) < \lambda_1(W_p^-)$---i.e., the even-sector perturbation is strictly more negative.
    The intra-even spectral gap $\lambda_2^+ - \lambda_1^+ \geq 8.69$ at $\lambda = 100$ (\Cref{lem:simplicity}, certified by interval arithmetic; growing monotonically to $\geq 731$ at $\lambda = 320{,}000$) exceeds the operator norm $\|W_p\|_{\mathrm{op}} < 0.46$ for $p \geq 100$ by a factor $\geq 18$.
    By a standard Rayleigh-quotient perturbation bound (see, e.g., \cite{Kato1995}, Theorem~V.4.10), this ensures that the leading eigenvector of the even block is stable under the rank-one update.
    Crucially, this transfers the Shift Parity Lemma from the \emph{isolated} prime matrix~$W_p$ to the \emph{full} operator: first-order perturbation theory gives $\delta\lambda_1^+ \approx \langle \eta^+ | W_p^+ | \eta^+ \rangle$ and $\delta\lambda_1^- \approx \langle \eta^- | W_p^- | \eta^- \rangle$, where $\eta^\pm$ are the ground-state eigenvectors of $A_\lambda^\pm$ (not of~$W_p^\pm$).
    The OP2 gap ($\geq 8.69$) ensures the second-order correction is bounded by $\|W_p\|_{\mathrm{op}}^2 / \mathrm{gap}_{\mathrm{intra}} < 0.46^2/8.69 < 0.025$, which is negligible.
    Since the stable eigenvectors $\eta^\pm$ are dominated by the leading Fourier mode (with overlap $> 0.99$ at $\lambda = 100$, increasing for larger~$\lambda$), the Rayleigh quotients $\langle \eta^\pm | W_p^\pm | \eta^\pm \rangle$ inherit the even-preference of the Shift Parity Lemma: $\delta\lambda_1^+ < \delta\lambda_1^-$.
    Hence each new prime \emph{deepens} the even--odd gap.

  \item \textbf{$L$-variation (Hellmann--Feynman).}
    Between consecutive primes, only $L = \log\lambda$ changes.
    The Hellmann--Feynman derivative (\Cref{rem:hellmann-feynman}) gives $\partial(\mathrm{gap})/\partial L > 0$ for $\lambda \geq 80$---this is an \emph{analytical} consequence of the operator structure (the $L$-derivative of the archimedean kernel shifts even modes more than odd modes), not a numerical observation at finitely many points.
    Hence the $L$-variation also deepens the gap.

  \item \textbf{Eigenvector stability (OP2 simplicity).}
    The certified intra-even gap (\Cref{lem:simplicity}) ensures that the identity of the ground-state eigenvector is preserved across all $33$ intervals.
    The perturbation from any single prime ($\|W_p\|_{\mathrm{op}} < 0.46$) is smaller than the certified intra-even gap ($\geq 8.69$) by the factor $\geq 18$ noted above; the analogous bound holds in the odd sector with even larger margin.
    This excludes level crossings within each sector.
\end{enumerate}

\noindent Since both mechanisms of change (new primes and $L$-variation) reinforce the existing even dominance, the gap remains strictly negative throughout $[\lambda_k, \lambda_{k+1}]$ for every adjacent pair of certificate values.

\emph{Regime~2} ($\lambda \geq 442{,}413$). \Cref{cor:M1-proved} establishes $\rho(\lambda) < 0$ for all $\lambda \geq \lambda_0$ (via the Euler--Maclaurin Proposition, now certified by interval arithmetic at $L = 13$, and the PNT Transfer Lemma). This gives $E_{\sin} < E_{\cos}$, hence
\[
...
\emph{Overlap.} Regimes~1 and~2 overlap in $[442{,}413,\, 1{,}300{,}000]$, where both the CAP certificates and the analytical argument apply. The overlap zone spans nearly a full order of magnitude, providing a robust bridge. The union covers all $\lambda \geq 100$.
\end{proof}
...
The spectral gap bound in \Cref{lem:higher-mode-decay} is derived analytically via the parity-split Laplace asymptotics (Step~3): $c_{\mathrm{gap}} \geq 4\pi^2/L \geq 2.8$ for $L \leq 14$, conservative by a factor $\geq 5$ compared to the earlier numerical estimate $c_{\mathrm{gap}} \geq 0.5$. Combined with the sector-difference control (\Cref{lem:resolvent-truncation}), no numerical constants remain in the Regime~2 argument.
\end{remark}
