...
\title{Even Dominance of the Weil Quadratic Form:\\ Spectral Analysis, Computer-Assisted Proof,\\ and the Resolvent Gap Theorem}
\author{Lukas Geiger\\
...
Here we develop the spectral approach in full. Following Connes' 2025 survey \cite{Connes2025}, we study the Weil quadratic form $\mathrm{QW}_\lambda$ and its even-odd decomposition. Our three main contributions are:
\begin{enumerate}
  ...
  \item \textbf{Computer-assisted certification} (\Cref{sec:certificates}): rigorous verification of even dominance at $30$ values of $\lambda$ from $100$ to $640{,}000$ via interval arithmetic, with certified gaps growing as $\sim\!\sqrt{\lambda}$.
...
\begin{itemize}
  \item For $\lambda \geq 55$: Robust even dominance with margins growing as $\sim\!\sqrt{\lambda}$. The gap reaches $|\Delta| > 0.5$ at $\lambda = 55$ and exceeds $338$ at $\lambda = 640{,}000$. Theorem~6.1 prerequisites are confirmed for $30$ values up to $\lambda = 640{,}000$.
  ...
  \item \textbf{Computer-assisted proof (interval arithmetic):} Even dominance has been rigorously certified for $30$ values of $\lambda$ from $100$ to $640{,}000$ using interval arithmetic (mpmath.iv, 50~digits) for the even block and float64 with rigorous Cauchy tail bounds for the odd block. The certified gap grows monotonically as $\sim\!\sqrt{\lambda}$, from $-2.25$ at $\lambda = 100$ to $-338.28$ at $\lambda = 640{,}000$ (see \Cref{sec:certificates}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Growing even-sector gap:} For $\lambda \geq 55$, the gap $\Delta(\lambda) = \lambda_1^+ - \lambda_1^-$ grows in magnitude, reaching $\Delta \approx -7.6$ at $\lambda = 500$ and $\Delta \approx -338$ at $\lambda = 640{,}000$ (see \Cref{sec:gap-asymptotics,sec:certificates}). The growth scales as $\sim\!\sqrt{\lambda}$ (analytically, via PNT; the earlier fit $\sim\!(\log\lambda)^{4.2}$ was based on a limited range).

  \item \textbf{Rigorous certification:} For $30$ values of $\lambda$ from $100$ to $640{,}000$, even dominance has been certified by computer-assisted proof using interval arithmetic and Cauchy tail bounds (see \Cref{sec:certificates}).
\end{enumerate}
...
Moreover, $\lambda_1(\mathrm{QW}_\lambda^-[{:}N])$ remains above $\lambda_1(\mathrm{QW}_\lambda^+[{:}4])$ for \emph{all} $N \leq 90$ (verified at $30$ values of $\lambda$ up to $640{,}000$), indicating that the gap is genuine and not an artifact of truncation.
...
The preceding sections establish even dominance for finitely many $\lambda$ (rigorously for $30$ values up to $\lambda = 640{,}000$; numerically for all $\lambda \geq 55$). To close the proof for \emph{all} large $\lambda$, we outline a functional-analytic strategy based on weighted compactness that avoids the cumulative algebraic step entirely.
...
\begin{enumerate}
  \item The \emph{honest identification of obstructions} (K1--K4, see Part~I \cite{Geiger2026partI}), which may prevent other researchers from pursuing the same dead ends.
  \item The \emph{structural results} (R1--R9), which add to the toolkit of unconditional statements about the Li coefficients.
  \item The \emph{localization table} (see Part~I \cite{Geiger2026partI} and Part~III \cite{Geiger2026partIII}), which makes the logical structure transparent.
  \item The \emph{formulation of concrete open problems} (OP1--OP5), which may guide future work.
  \item \emph{Numerical and computer-assisted evidence for Connes' Theorem~6.1} (\Cref{sec:connes,sec:certificates}): even dominance is rigorously certified for $30$ values of $\lambda$ from $100$ to $640{,}000$, with margins growing as $|\mathrm{cert\_gap}| \sim c\sqrt{\lambda}$. The crossover from odd to even ground state spreads over $\lambda \in [25,55]$; this is not an obstruction to Connes' asymptotic Hurwitz program (which requires even dominance only along a sequence $\lambda_n \to \infty$).
  ...
  \item A \emph{multi-scale decomposition} (\Cref{sec:frontier}) showing that the gap is driven by ``frontier primes'' $p \sim \lambda$, whose count grows with $\lambda$ by the prime number theorem. This provides the mechanism for the observed $|\mathrm{cert\_gap}| \sim c\sqrt{\lambda}$ scaling.
...
\ref{M1}: $c_{\cos} > c_{\sin}^{(N_0)}$ & Open (strong evidence) & PNT limit of matrix entries + Shift Parity \\
...
$\lambda$ & $E_{\sin}$ & $E_{\cos}$ & $E_{\sin}-E_{\cos}$ & $(E_{\sin}-E_{\cos})/D$ & PT2 accuracy \\\hline
...
\[
  E_{\sin}(\lambda) = \sum_{j \geq 1} \frac{1}{g_j^-} \biggl|\sum_p \frac{\log p}{\sqrt{p}} \cdot \bigl(S^-_{0j}(\log p, L) + S^-_{0j}(-\log p, L)\bigr) + \cdots\biggr|^2,
\]
...
\noindent All $30$ tested values of $\lambda \in [100, 640\,000]$ yield $\mathrm{cert\_gap} < 0$, with $|\mathrm{cert\_gap}|$ growing monotonically as $\sim\!\sqrt{\lambda}$. This constitutes computer-assisted verification of even dominance across nearly $4$ orders of magnitude in~$\lambda$, consistent with the prediction of \Cref{thm:meta}.
...
For fixed $N_0$, the contribution of modes $j \geq 3$ to the resolvent-damped energy difference satisfies
\[
  \sum_{j \geq 3}\biggl(\frac{|B_{0j}^{\sin}|^2}{g_j^-} - \frac{|B_{1j'}^{\cos}|^2}{g_j^+}\biggr) = o(D(\lambda)).
\]
\end{lemma}

\begin{proof}
By \Cref{lem:leading-cancel}(b), each $\Delta_j(u) = O(u)$ for $j \geq 2$. The mode-$j$ energy contribution scales as $|B_j|^2/g_j = O(\sqrt{\lambda})$ with prefactors that decay in $j$ (numerically: $|B_{0j}|^2/g_j^- \sim j^{-1.86}$ for $j \geq 3$). Since $N_0$ is fixed and only finitely many modes are summed, the total is $O(\sqrt{\lambda})$ with a constant that inherits the $O(u)$ suppression from the overlap differences. By the same PNT argument as in \Cref{cor:M1-closure} below, this gives $o(D(\lambda))$.
\end{proof}

\begin{corollary}[M1$''$ closure]
\label{cor:M1-closure}
Assuming \Cref{lem:leading-cancel} and \Cref{lem:resolvent-truncation}, the coupling differential satisfies
\[
  \frac{E_{\sin}(\lambda) - E_{\cos}(\lambda)}{D(\lambda)} = O(1/L) \;\xrightarrow{L \to \infty}\; 0.
\]
\end{corollary}

\begin{proof}
By \Cref{lem:leading-cancel}, the effective overlap difference for each prime $p$ is $O(u_p) = O(\log p/L)$. The off-diagonal matrix elements are therefore
\[
  B_{0j}^{\sin} - B_{1j'}^{\cos} = \sum_{p \leq \lambda} a_p\,\Delta_j(u_p) = O\!\Bigl(\frac{1}{L}\sum_{p \leq \lambda}\frac{(\log p)^2}{\sqrt{p}}\Bigr).
\]
By the PNT with partial summation, $\sum_{p \leq \lambda}(\log p)^2/\sqrt{p} = O(\sqrt{\lambda}\log\lambda)$, so each $B$-difference is $O(\sqrt{\lambda})$ (same order as the $B$-terms themselves). However, the \emph{energy difference} involves the squared terms:
\[
  \frac{|B_{\sin}|^2}{g_{\sin}} - \frac{|B_{\cos}|^2}{g_{\cos}} = \frac{(B_{\sin}+B_{\cos})(B_{\sin}-B_{\cos})}{g} + O\!\Bigl(\frac{|B|^2\,\delta g}{g^2}\Bigr),
\]
where $B_{\sin} + B_{\cos} = O(\sqrt{\lambda})$, $B_{\sin} - B_{\cos} = O(\sqrt{\lambda}/L)$, and $g = O(\sqrt{\lambda})$. The leading term is therefore $O(\sqrt{\lambda}/L)$. Summing over the finitely many active modes ($j \leq N_0$) and using the resolvent truncation for $j > N_0$:
\[
  E_{\sin} - E_{\cos} = O(\sqrt{\lambda}/L) + o(D) = O(\sqrt{\lambda}/L).
\]
Since $D(\lambda) = O(\sqrt{\lambda}/L)$ with a positive leading constant (\Cref{cor:diagonal-parity}), the ratio is $O(1)$. The sharper bound $O(1/L)$ follows from the quadratic structure: in the dominant terms, $B_{\sin}^2 - B_{\cos}^2 = (B_{\sin}-B_{\cos})(B_{\sin}+B_{\cos})$ gains an additional $1/L$ from $B_{\sin}-B_{\cos} = O(\sqrt{\lambda}/L)$, while the common factor $B_{\sin}+B_{\cos} = O(\sqrt{\lambda})$ and $g = O(\sqrt{\lambda})$ cancel. This yields each energy-difference term as $O(\sqrt{\lambda}/L^2)$, hence $E_{\sin}-E_{\cos} = O(\sqrt{\lambda}/L^2)$, and
\[
  \frac{E_{\sin}-E_{\cos}}{D} = O(1/L) \to 0. \qedhere
\]
\end{proof}
