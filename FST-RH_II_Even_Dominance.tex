...
\begin{remark}[Status of M1]
\label{rem:M1-status}
The certified gap $\mathrm{cert\_gap}(\lambda) < 0$ is \emph{rigorously verified} for $30$ values of $\lambda$ up to $640{,}000$. The analytical framework establishes:
\begin{itemize}
  \item The gap is a \emph{second-order perturbative effect} in the $1/L$ expansion (\Cref{rem:gap-source}): it vanishes at both zeroth and first order.
  \item The numerator difference $|B_j^-|^2 - |B_j^+|^2$ is controlled by the Leading-Mode Cancellation (\Cref{lem:leading-cancel}).
  \item The gap ratio $g_j^-/g_j^+$ is bounded away from zero (both gaps grow as $O(\sqrt{\lambda})$), but the ratio is $\approx 0.15$--$0.39$, not close to unity.
  \item The ratio $\rho(\lambda) = (E_{\sin} - E_{\cos})/D$ decreases monotonically from $0.63$ to $0.14$ over $\lambda = 100$ to $3000$.
\end{itemize}
The remaining analytical step is to prove $\rho(\lambda) < 1$ for all $\lambda \geq \lambda_0$. This is a \emph{second-order} spectral perturbation problem: the $1/L$-expansion of the resolvent-damped energies in the degenerate $\lambda_1 = -1$ eigenspace. The numerical evidence ($30$ certificates, monotone $\rho$) is strong, but a rigorous proof remains open.
\end{remark}
