...
\begin{itemize}
  ...
  \item The gap ratio $g_j^-/g_j^+$ is bounded away from zero (both gaps grow as $O(\sqrt{\lambda})$).
  \item The ratio $\rho(\lambda)$ decreases monotonically from $0.63$ to $0.28$ over $\lambda = 100$ to $3000$.
\end{itemize}
The remaining analytical step is to prove that $\rho(\lambda) < 1$ for all $\lambda \geq \lambda_0$. This requires a quantitative bound on the second term in~\eqref{eq:direct-decomp} that accounts for the gap asymmetry. The numerical evidence strongly suggests $\rho(\lambda) \to 0$, but a rigorous proof of this convergence remains open.
\end{remark}
